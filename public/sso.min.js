...
function _a() {
    _h('POST', window.ysgz.baseUrl + '/sso/auth?hytoken=' + window.localStorage.getItem('hytoken'), function (d) {
        if (d.returnCode == '003') {
            window.location.href = ysgz.loginUrl + '?backurl=' + encodeURIComponent(window.location.href)
        } else {
            window.localStorage.setItem('hytoken', d.data);
        }
    });
};
_h('GET', window.ecpJson, function (d) {
    window.ysgz = d;
    window.ysgz.baseUrl = window.location.origin + "/ysgz";

    if (d.ssoLock) {
        if (window.localStorage.getItem('hytoken')) {
            hookAjax({
                open: function (arg, xhr) {
                    ...
                    if (arg[1].indexOf('?') > 0) {
                        arg[1] += '&hytoken=' + window.localStorage.getItem('hytoken');
                    } else {
                        arg[1] += '?hytoken=' + window.localStorage.getItem('hytoken');
                    }
                },
            ...
            window.ysgz.loginUrl = window.location.origin + "/ysgz";
